...
date <- "Aug.2024"
# - BD vs. HC DMPs 
DMPs <- read.csv(file.path(inputFolder,"EWAS-BB-BDvsHC_DMPs_Aug.2024.csv"), row.names = 1)

# - BD vs. HC DMPs svaPCs - Accounting for smoking
DMPsSVA <- read.csv(file.path(inputFolder,"EWAS-BB-BDvsHC_DMPs_svaPCs_Aug.2024.csv"), row.names = 1)
...
### Manhatam
```{r}
BDvsHC_ManhattanPlot <- paste0("EWAS-BB-BDvsHC_ManhattanPlot_",date,".png")
BDvsHC_svaPCs_ManhattanPlot <- paste0("EWAS-BB-BDvsHC_svaPCs_ManhattanPlot_",date,".png") 
```

### QQplot 
```{r}
BDvsHC_QQplot <- paste0("EWAS-BB-BDvsHC_QQplot_",date,".png")
BDvsHC_QQplot_svaPCs <- paste0("EWAS-BB-BDvsHC_QQplot_svaPCs_",date,".png")
```
### Volcano Plot
```{r}
BDvsHC_FDR_Corrected_VolcanoPlot <- paste0("EWAS-BB-BDvsHC_FDR-Corrected_VolcanoPlot",date,".png")
BDvsHC_FDR_Corrected_svaPCs_VolcanoPlot <- paste0("EWAS-BB-BDvsHC_FDR-Corrected_svaPCs_VolcanoPlot",date,".png")
```
...
g + 
  ...
  ylim(0,10)
ggsave(filename = BDvsHC_ManhattanPlot ,path = file.path(outputFolder,"2.ManhattanPlots/"), device='png', dpi=300, width = 10, height = 7)
...
g + 
  ...
  geom_hline(yintercept = EWSthresh, color = "red", linetype = "dashed") +
  geom_hline(yintercept = FDRthres, color = "blue4", linetype = "longdash") +
  ggtitle("BD vs. HC Manhattan Plot - svaPCs ") +
  theme(plot.title = element_text(size = 15, face = "bold", family = "Segoi U")) +
  ylim(0,10)
ggsave(filename = BDvsHC_svaPCs_ManhattanPlot,path = file.path(outputFolder,"2.ManhattanPlots/"), device='png', dpi=300, width = 10, height = 7)
```
# BD vs. HC QQplot
### NO SVA
```{r}
...
ggsave(filename = BDvsHC_QQplot,path = file.path(outputFolder,"1.QQplots/"), device='png', dpi=300, width = 7, height = 7)
...
qq + ggtitle("BD vs. HC QQPlot - svaPCs ") +
  ...
  annotate("text", x = Inf, y = Inf, label = paste("lambda GC =", round(lambda_gc, 3)), 
           hjust = 1.1, vjust = 1.1, size = 5, color = "red")
ggsave(filename = BDvsHC_QQplot_svaPCs,path = file.path(outputFolder,"1.QQplots/"), device='png', dpi=300, width = 7, height = 7)
```
...
#   xlim = c(-1.8,1.8),
#   labSize = 2.0)
# BDvsHC_EnhVolc + ggtitle("BD vs. HC Enhanced Volcano FDR-Corrected p-value", 
#                          subtitle = "") +
#   theme(plot.title = element_text(size = 15, face = "bold", family = "Segoi U"))
...
# ggsave(filename = BDvsHC_FDR_Corrected_VolcanoPlot,path = file.path(outputFolder,"3.VolcanoPlots"), device='png', dpi=300, width = 10, height = 7)
...
BDvsHC_ggplot = ggplot(data=DMPs_sorted, aes(x=logFC, y=-log10(P.Value))) +
...
BDvsHC_ggplot + ggtitle("BD vs HC - Volcano Plot", 
                         subtitle = "") +
  theme(plot.title = element_text(size = 15, face = "bold", family = "Segoi U"))
ggsave(filename = BDvsHC_FDR_Corrected_VolcanoPlot,path = file.path(outputFolder,"3.VolcanoPlots"), device='png', dpi=300, width = 10, height = 7)
```
...
#   theme(plot.title = element_text(size = 15, face = "bold", family = "Segoi U"))
# #ggsave(paste0("VolcanoPlot_BD_v_HC_FDR-corrected",date,".png"),path = "V:/common/Walss-Bass_Lab/Walss-Bass lab juliet/Iago/1.EWAS_K01_Analysis/Aug.24-EWAS_BDvsHC_BRAINBANK/2.Output_EWAS-models/3.VolcanoPlots", device='png', dpi=300, width = 10, height = 7)
# ggsave(filename = BDvsHC_FDR_Corrected_svaPCs_VolcanoPlot,path = file.path(outputFolder,"3.VolcanoPlots"), device='png', dpi=300, width = 10, height = 7)
...
# load library
# plot adding up all layers we have seen so far
BDvsHC_ggplot = ggplot(data=DMPsSVA_sorted, aes(x=logFC, y=-log10(P.Value))) +
        geom_point(aes(color=ifelse(-log10(P.Value) > -log10(9e-8), "above", "below")), show.legend = FALSE) + 
